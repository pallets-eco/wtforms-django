...
class TemplateTagsTest(TestCase):
    ...
    def _render(self, source):
        ...
        return t.render(
            Context({"form": self.F(), "a": self.F().a, "someclass": "CLASSVAL>!"})
        )

    def test_simple_print(self):
        self.assertEqual(
            self._render("{% autoescape off %}{{ form.a }}{% endautoescape %}"),
    ...
    def test_non_templates_calling(self):
        if django.VERSION < (1, 4):
            return
        self.assertEqual(
            self._render("{% autoescape off %}{{ form.a.name }}{% endautoescape %}"),
            "a",
        )
        self.assertEqual(
            self._render("{% autoescape off %}{{ form.a.label }}{% endautoescape %}"),
            '<label for="a">I r label</label>',
        )

    def test_form_field(self):
        self.assertEqual(
            self._render("{% form_field form.a %}"),
            '<input id="a" name="a" type="text" value="">',
        )
        self.assertEqual(
...
class QuerySetSelectFieldTest(DjangoTestCase):
    ...
    def setUp(self):
        ...
        class F(Form):
            a = QuerySetSelectField(
                allow_blank=True, get_label="name", widget=lazy_select
            )
    ...
    def test_get_label_alt(self):
        class TestForm(Form):
            a = QuerySetSelectField(
                ...
                widget=lazy_select,
                get_label=lambda x: x.name.upper(),
            )
